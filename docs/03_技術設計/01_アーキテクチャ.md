...
graph TB
    ...
    subgraph Vercel
        ...
        WORKER["非同期ジョブWorker<br/>after() + jobs テーブル"]
...
| LLM | OpenAI 互換 API（OpenRouter / DeepSeek / OpenAI） | `openai` SDK | `LLM_BASE_URL` + `LLM_MODEL_*` の差し替えで切替 |
| Embedding | OpenAI 互換 `/embeddings`（既定 text-embedding-3-small） | `openai` SDK | 1536次元（`EMBEDDING_DIM`） |
| 非同期ジョブ | Supabase jobs テーブル + Next.js `after()` | `next/server` | Cron 不使用（DEC-13）。将来 Inngest 移行可 |
...
sequenceDiagram
    ...
    Note over Worker: ACK 直後に after() で起動（Cron 不使用・DEC-13）
    Worker->>Jobs: pending jobを1件取得（CAS で claim）
