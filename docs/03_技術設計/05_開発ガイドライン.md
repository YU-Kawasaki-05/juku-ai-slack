...
実装は `src/shared/lib/env.ts`。Zod スキーマで検証し、失敗したらモジュール読み込み時に throw する。
`import 'server-only'` でクライアントバンドルへの誤取り込みをビルド時に検知する。
...
const envSchema = z.object({
  ...
  // 任意（未設定でも起動する。対応機能だけが無効化される）
  LLM_API_KEY: z.string().min(1).optional(),
  // …一覧は docs/03_技術設計/03_外部サービス.md 4章
})
...
> **LLM 系を必須にしない理由**: 未設定で env のパースが落ちると Slack 連携ごと 500 になり、
> エラー文言すら返せなくなる。実行時に LLM を呼ぶ箇所で存在チェックし、
> `AI_RESPONSE_FAILED` として生徒に定型文を返す設計にしている。
