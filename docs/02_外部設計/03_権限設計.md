...
- **ロールの付与は必須**。`app_metadata.role` が `staff` / `admin` のいずれでもないアカウントは、
  ログインはできるが管理画面を一切利用できない（下記 2.2）

### 2.2 実装との対応（アプリ層のガード）

| ガード | 判定 | 失敗時 |
|--------|------|--------|
| `requireStaff()` | `app_metadata.role` が `staff` または `admin`（admin は staff の上位） | 未認証は `unauthorized` / ロール未設定は `forbidden` を throw |
| `requireStaffPage()` | 同上（Server Component 用） | 未認証は `/login`、ロール未設定は `/admin/no-access` へリダイレクト |
| `requireAdmin()` | `app_metadata.role === 'admin'` | 未認証は `unauthorized` / staff・ロール未設定は `forbidden` を throw |

- EP-02〜EP-18 の "admin または staff" は `requireStaff` が、EP-14 / DEC-15 の admin 限定は
  `requireAdmin` が実装する（`src/shared/lib/auth/`）
- **EP-07〜09（チャンネル紐付け）は staff 可**。admin 限定から開放した経緯と、代わりに入れた
  防御は下記 3.1 を参照
- 管理画面は Service Role で DB を読み RLS（migration 026）をバイパスするため、
  **このアプリ層のロール検証が唯一の防御線**。「ログインできる ＝ 見られる」にしてはならない
  （受け入れテスト AT-05 / AT-05b が回帰を検出する）
- ロール未設定のユーザーを `/login` に戻すと「ログイン済みなのにログイン画面に戻される」
  無限ループに見えるため、専用ページ `/admin/no-access`（理由・付与手順・ログアウト導線）へ案内する
...
### 3.1 EP-07〜09（チャンネル紐付け）を staff に開放した理由と、代わりの防御

**経緯**: 当初は admin 限定だった。紐付けを誤ると**別生徒のプロフィールとレポートで AI が
回答する**事故になるため、操作できる人を絞るのが最も簡単な防御だったから。

**変更した理由（運用要件）**: 本番稼働後、**生徒チャンネルの紐付けは担当スタッフが行う**運用に
なった。生徒が増えるたび admin に依頼する形は、(a) 紐付け待ちの生徒が Bot を使えない期間が
生まれる、(b) admin が「どのチャンネルがどの生徒か」を知らないまま代行するため、
依頼の取り違えという別の事故経路ができる、という 2 つの問題があった。
**担当スタッフ本人が操作するほうが、生徒とチャンネルの対応を知っている人が入力する**ぶん
正確になる。

**入口の制限を外した代わりに入れた防御**（`requireAdmin` 1 つを 3 つで置き換える）:

| # | 防御 | 実装 | 何を防ぐか |
|---|------|------|-----------|
| 1 | 確定前の確認ダイアログ | `BindingForm`（`#チャンネル名` と**生徒名**を明示し、「この生徒の学習履歴とレポートが使われます」と結果を説明） | 生徒 Select の取り違えに、保存前に気づける |
| 2 | 既定レポートの生徒一致検査 | `createBindingAction`（`reports.person_id !== personId` を拒否。BR-05-11 と同じ隔離要件） | 他生徒のレポートが既定レポートとして混入する |
| 3 | 作成・変更の操作ログ | `createBindingAction` / `updateBindingAction` → `logError(severity='info')`。コードは `CHANNEL_BINDING_CREATED` / `CHANNEL_BINDING_UPDATED`（`07_エラー文言設計.md`） | 事故が起きたとき「誰がいつどの紐付けをしたか」が追えない状態 |

操作ログには**操作者のメールアドレス・channel_id・person_id・status**を残す。更新時は
`person_id_before` / `person_id_after` を並べて記録し、**付け替わっていないこと自体**が
後から検証できるようにする。`dedupeWhileUnresolved` は使わない（操作ログは毎回 1 行残す）。

**変わっていない前提**:
- `person_id` は紐付け後に変更できない（BR-15-01）。付け替えが必要なら紐付けを作り直す。
  したがって「編集で別の生徒に付け替える」導線は存在しない
- `channel_id` も変更不可。`channel_id` が「誰の質問か」を決める信頼の基点（BR-07-01）
- admin 限定のまま残す操作: Embedding 手動再生成（EP-14）、AI 応答の停止・再開（DEC-15）。
  どちらも**全生徒に影響する / コストが発生する**操作で、担当スタッフ単位の業務ではない
