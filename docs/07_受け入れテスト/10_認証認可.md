...
2. `/login` からログイン
3. スタッフ用画面 `/admin/channels/new`（EP-07〜09。staff 可だがロールなしは不可）を開く
...
## AT-13 staff はチャンネル紐付けを作成でき、操作ログが残る

**優先度**: 必須 / **実施方法**: acceptance spec / **要件**: EP-07〜09 / 権限設計 3.1

> **2026-09-04 変更**: 以前は「staff はチャンネル紐付けを作成できない」（admin 限定, D-3）を
> 検証していた。生徒チャンネルの紐付けを担当スタッフが行う運用に変わり、EP-07〜09 を
> staff 可に開放した（理由は `docs/02_外部設計/03_権限設計.md` 3.1）。
> **入口を閉じる防御が無くなったので、代わりの防御が効いていることをこのケースで検証する。**

### 目的

`channel_id` は「誰の質問か」を決める信頼の基点。紐付けを誤ると
**別生徒のプロフィールとレポートで AI が回答する**。staff に開放した代わりの防御——
(1) 確定前に生徒名を明示する確認ダイアログ、(2) 誰がどの紐付けをしたかの操作ログ——が
実際に機能していることを確認する。

### 手順

staff でログインし `/admin/channels/new` でフォームを埋めて「紐付ける」を押し、
確認ダイアログの内容を確認してから「紐付けを確定する」。

### 期待結果

- 確認ダイアログに **`#チャンネル名`**・**生徒名**・「この生徒の学習履歴とレポートが使われます」が出る
- 確定後、一覧にその生徒名の行が出る（staff でも作成できる）
- `ai_error_logs` に `error_code='CHANNEL_BINDING_CREATED'` / `severity='info'` の行が残り、
  `internal_message` に `actor=<操作者のメール>` と `person_id=<生徒>` が含まれる
...
![AT-13](evidence/AT-13_staffはチャンネル紐付けを作成できる.png)

`evidence/AT-13_staffはチャンネル紐付けを作成できる.png`
