...
def check_reasoning_off(cfg: dict, model: str) -> dict:
    ...
    for name, extra in candidates:
        ...
        res = r["res"]
        if res["status"] != 200:
            rows.append({"param": name, "accepted": False, "error": api_error(res)[:160]})
            ng("%-32s \u2192 HTTP %s: %s" % (name, res["status"], api_error(res)[:100]))
            continue
        u = usage_of(res)
        rt = reasoning_tokens_of(u)
        body = content_of(res)
        rows.append({"param": name, "accepted": True, "reasoning_tokens": rt,
...
def measure_prompt_tokens(cfg: dict, model: str, parts: list, label: str) -> dict:
    ...
    if res["status"] == 400 and not r["swapped"]:
        ...
        res = r["res"]
    if res["status"] != 200:
        return {"label": label, "ok": False, "http": res["status"], "error": api_error(res)}
    u = usage_of(res)
    return {"label": label, "ok": True, "prompt_tokens": int(u.get("prompt_tokens") or 0),
            "ms": res["ms"], "content": content_of(res)}
...
def check_detail(cfg: dict, model: str, baseline: int) -> dict:
    ...
    for label, value, why in DETAIL_VARIANTS:
        ...
        if not m["ok"]:
            rows[label] = {"accepted": False, "http": m["http"], "error": m["error"]}
            ng("detail=%s \u2192 HTTP %s : %s" % (display_pad(label, 9), m["http"], (m["error"] or "")[:90]))
            info("      %s" % why)
            continue
        delta = (m["prompt_tokens"] - baseline) if baseline is not None else None
        rows[label] = {"accepted": True, "prompt_tokens": m["prompt_tokens"], "image_tokens": delta}
        ok("detail=%s \u2192 \u53d7\u4ed8 OK / prompt_tokens=%d / \u753b\u50cf\u5206=%s"
           % (display_pad(label, 9), m["prompt_tokens"], delta if delta is not None else "\u4e0d\u660e"))
        info("      %s" % why)

    print("")
    accepted = [k for k, v in rows.items() if v.get("accepted")]
    rejected = [k for k, v in rows.items() if not v.get("accepted")]
...
def check_image_sizes(cfg: dict, model: str, baseline: int, known: dict) -> dict:
    ...
    for s in sizes:
        if s in known:
            rows[str(s)] = {"image_tokens": known[s], "reused": True}
        ...
        if not m["ok"]:
            rows[str(s)] = {"error": m["error"], "http": m["http"]}
            ng("%4dx%-4d \u2192 HTTP %s : %s" % (s, s, m["http"], (m["error"] or "")[:90]))
            continue
        delta = (m["prompt_tokens"] - baseline) if baseline is not None else None
        rows[str(s)] = {"image_tokens": delta, "prompt_tokens": m["prompt_tokens"], "png_bytes": len(png)}
    ...
    print("")
    out = {"status": "ok" if vals else "fail", "by_size": rows}
    if len(vals) >= 2:
...
def print_summary(cfg: dict, res_chat: dict, res_vision: dict, res_emb: dict,
                  res_sizes: dict, res_detail: dict) -> None:
    hr("\u307e\u3068\u3081")
    ...
    print("  " + "-" * 66)
    d_ok = res_chat.get("default", {}).get("status") in ("ok", "warn")
    c_ok = res_chat.get("complex", {}).get("status") in ("ok", "warn")
    e_ok = res_emb.get("status") == "ok"
    lines = [
    ...
    marks = ["\u2705", "\u2705" if d_ok else "\u274c", "\u2705" if c_ok else "\u274c", "\u2705", "\u2705" if e_ok else "\u274c"]
    for mark, line in zip(marks, lines):
        print("      %s %s" % (mark, line))
    print("      \u2139  LLM_API_KEY / EMBEDDING_API_KEY \u306f\u540c\u3058\u30ad\u30fc\u3092\u4f7f\u3048\u307e\u3059\uff08\u3053\u3053\u306b\u306f\u51fa\u3057\u307e\u305b\u3093\uff09")
    if not (d_ok and c_ok and e_ok):
        print("      \u26a0\ufe0f  \u274c \u304c\u4ed8\u3044\u305f\u884c\u306f\u305d\u306e\u307e\u307e\u4f7f\u3048\u307e\u305b\u3093\u3002\u2461 \u306e\u30e2\u30c7\u30eb\u4e00\u89a7\u304b\u3089\u6b63\u3057\u3044 ID \u3092\u63a2\u3057\u3066\u304f\u3060\u3055\u3044\u3002")
    RESULT["summary"]["env"] = {"lines": lines, "all_ok": bool(d_ok and c_ok and e_ok)}
    ...
    if not tokens:
        warn("\u753b\u50cf 1 \u679a\u306e\u30c8\u30fc\u30af\u30f3\u6570\u3092\u6e2c\u5b9a\u3067\u304d\u307e\u305b\u3093\u3067\u3057\u305f\u3002\u2463\u2466 \u306e\u30a8\u30e9\u30fc\u3092\u898b\u3066\u518d\u5b9f\u884c\u3057\u3066\u304f\u3060\u3055\u3044\u3002")
...
def main() -> int:
    ...
    guarded("\u2461 \u30e2\u30c7\u30eb\u5b9f\u5728\u78ba\u8a8d", check_models, cfg, auth.get("_ids"))
    ...
    fails = [c for c in RESULT["checks"].values()
